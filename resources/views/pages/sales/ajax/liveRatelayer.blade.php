@if(!empty($rate))
<div class="container-fluid" id="layer">
    <div class="row justify-content-center">
        <div class="col-md-7">
            <div class="card border-success my-4">
                <form class="" id="" action="{{ route('layer.sales') }}" method="POST">      
                    @csrf  

                    <div class="card-header bg-transparent border-success">
                        Live rate layer
                        <input type="hidden" name="rate" id="layer_rate" value="{{ $rate['Live rate layer'] }} "/>
                        <input type="hidden" name="date" value="{{ $rate['Date'] }} "/>
                        <input type="hidden" name="customer" id="layer_customer" />
                        <small class="float-end font-weight-bold text-danger"> @convert($rate['Live rate layer']) </small>
                    </div>
                    <div class="card-body text-success">
                        ...
                        <div class="mb-3">
                            <label for="layer_weight" class="form-label">Weight</label>
                            <input  type="number" name="weight" class="form-control" id="layer_weight" aria-describedby="layer_weightHelp">
                            <div id="layer_weightHelp" class="form-text">Weight will be calculated in kg .</div>
                        </div>
                                             
                        <div class="row">
                            <div class="col">
                                <label for="layer_totalCost" class="form-label">Total cost</label>
                                <input name="amount" class="form-control" disabled  id="layer_totalCost" aria-describedby="layer_costHelp" />                                
                            </div>
                            <div class="col">
                                <label for="layer_totalCost" class="form-label">Receive cost</label>
                                <input name="receive" class="form-control" id="layer_receiveCost"/>                                
                            </div>
                            <div id="layer_costHelp" class="form-text mb-4">Cost will be calculated in {{ config('airtable.currency')}} .</div>                            
                        </div>  
        ...
        <div class="col-md-5">
            <div class="card border-success my-4">
                ...
                <div class="card-body text-success p-0">                                  
                    ...
                    <!--  -->
                    <a href="{{ route('view.all.sales.data').'?table=Layer Sales' }}" class="dynamicModal btn btn-secondary m-2 float-end"> Close Today Sale of  Layer</a>
                </div>                              
            </div>
        </div>
    </div>
...
<script>
    $(document).ready(function(){
        // 
        $("#layer_weight").on("keyup",function(){
            $rate = $("#layer_rate").val();
            $weight = $("#layer_weight").val();
            if($weight != "" && $weight != null){
                $cost = parseFloat($rate) * parseFloat($weight);                    
                $("#layer_totalCost").val($cost);
            }
        });
        ...
        $('.phone_us').mask('(000) 000-0000',{
            onComplete: function(phone) {
                $.ajax({
                    ...
                    dataType:'json',
                    data:{phone:phone,table:'Layer Sales'},
                    ...
                    success:function(res){
                        ...
                        if(_.size(res.customer)){
                            ...
                            $("#layer_customer").val($customer.id);
